...
# EFS (TLS + IAM auth via amazon-efs-utils; CDK denyAnonymousAccess flag
# requires IAM auth to mount, not just SG-based)
mount_or_skip "$MOUNT_EFS" \
  mount -t efs -o tls,iam "$EFS_FS_ID":/ "$MOUNT_EFS"

# S3 Files (NFSv4.x via amazon-efs-utils 's3files' type)
mount_or_skip "$MOUNT_S3FILES" \
  mount -t s3files "$S3FILES_FS_ID":/ "$MOUNT_S3FILES"

# Mountpoint for S3 (FUSE). v1.22.3+ default is no client cache;
# the legacy --no-cache flag is removed (use --cache <dir> to opt in).
mount_or_skip "$MOUNT_MOUNTPOINT" \
  mount-s3 "$BUCKET_B" "$MOUNT_MOUNTPOINT" \
    --allow-other --region "$AWS_REGION"
