...
[[ -z "$s3files" ]] && note "  (none)" || fail "  S3Files: $s3files"
...
lambdas=$(aws lambda list-functions --region "$REGION" \
  ...
  --output text 2>/dev/null || true)
[[ -z "$lambdas" ]] && note "  (none)" || fail "  Lambda: $lambdas"
