...
| # | 패턴 | 대체 대상 | 핵심 인사이트 | 상태 |
|---|------|----------|-------------|------|
| 1 | **Key-Value Store** | DynamoDB | S3 객체 키 = 당신의 키, 객체 바디 = 당신의 값. 50만 키를 비용 효율적으로 처리할 수 있을까? | 📋 리서치 완료 |
| 2 | **S3 as Event Store** | Kinesis / SQS | S3 Event Notifications를 내구성 있는 리플레이 가능한 이벤트 로그로 활용. | 📋 리서치 완료 |
| 3 | **Litestream + SQLite** | RDS (소규모) | 인메모리 DB 속도 + [Litestream](https://github.com/benbjohnson/litestream)을 통한 S3 내구성. Fargate Spot RTO/RPO 실험. | 📋 리서치 완료 |
| 4 | **Serverless RDBMS** | RDS / Aurora | S3의 Parquet + Athena = 데이터베이스 서버 없이 SQL 쿼리. 수 초를 허용할 수 있다면, 왜 RDS에 비용을 쓸까? | 📋 리서치 완료 |
| 5 | **S3 as File I/O** | EBS / EFS | S3 API의 읽기/쓰기는 로컬 파일시스템과 비교해 어떨까? 파일 크기별 성능 프로파일링. | 📋 리서치 완료 |
| 5+ | **S3 Files vs Mountpoint vs EFS** (cold-cache) | — | AWS S3 Files "1ms 이하" 마케팅 수치 검증. fio 4종 × 3 시스템 cold-only + SageMaker 마운트 호환성 PoC. | ✅ 측정 완료 — [결과 보고서](./docs/research/s3-files.md) |

> 각 패턴의 PoC 구현은 진행 중입니다. 현재까지의 리서치와 벤치마크 결과는 [docs/research/](./docs/research/)에서 확인할 수 있습니다.
